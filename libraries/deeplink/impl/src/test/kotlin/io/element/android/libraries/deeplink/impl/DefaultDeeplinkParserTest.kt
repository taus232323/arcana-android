...
class DefaultDeeplinkParserTest {
    companion object {
        const val A_URI =
            "arcana://open/@alice:server.org"
        const val A_URI_WITH_ROOM =
            "arcana://open/@alice:server.org/!aRoomId:domain"
        const val A_URI_WITH_ROOM_WITH_THREAD =
            "arcana://open/@alice:server.org/!aRoomId:domain/\$aThreadId"
        const val A_URI_WITH_ROOM_WITH_THREAD_AND_EVENT =
            "arcana://open/@alice:server.org/!aRoomId:domain/\$aThreadId/\$anEventId"
        const val A_URI_WITH_ROOM_WITH_EVENT_AND_NO_THREAD =
            "arcana://open/@alice:server.org/!aRoomId:domain//\$anEventId"
        const val A_URI_WITH_ROOM_WITH_THREAD_AND_EVENT_AND_INVALID_CHARACTERS =
            "arcana://open/@a%2Flice:server.org/!a%2FRoomId:domain/\$a%2FThreadId/\$an%2FEventId"
    }
    ...
    fun `error cases`() {
        ...
        // Bad host
        assertThat(sut.getFromIntent(createIntent("arcana://close/@alice:server.org"))).isNull()
        // No session Id
        assertThat(sut.getFromIntent(createIntent("arcana://open"))).isNull()

        assertThrowsInDebug {
            // Invalid sessionId
            sut.getFromIntent(createIntent("arcana://open/alice:server.org"))
        }
        assertThrowsInDebug {
            // Empty sessionId
            sut.getFromIntent(createIntent("arcana://open//"))
        }
    }
